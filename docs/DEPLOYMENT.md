...
| `ANTHROPIC_API_KEY` | your Anthropic API key | **apps/portal only** — powers the Owner AI Assistant (`/owner/ai`), called exclusively from server routes/Server Actions per `docs/ARCHITECTURE.md` §D ("the key never reaches the browser"). Leaving it unset keeps the AI Assistant in its labelled demo/canned-response mode instead of erroring |
